function to_seg_fits=segmented_fit_to(anal_opts_fit_to,data)
...
% CUSTOM FOR DEV
anal_opts_fit_to.setpt_range = [3.628698368419995e+08 362871100];
anal_opts_fit_to.scan_length = 39;
anal_opts_fit_to.start_scan = 41;

setpts=data.wm_log.proc.probe.freq.set;
...
scan_length = median(diff(scan_edges))-1;
num_shots = length(data.mcp_tdc.shot_num);
iimax = ceil(num_shots/scan_length);
...
for ii=1:iimax
    ...
    if ii == iimax
        ...
        if seg_end - scan_length < 1
            seg_start = 1;
        else
            seg_start = seg_end - scan_length;
        end
    end
    seg_mask = [zeros(seg_start-1,1);ones(seg_end-seg_start,1);zeros(num_shots-seg_end,1)];
    set_sel = setpts(seg_mask==1);
    shot_idx = seg_start:seg_end-1;
    plot(shot_idx,set_sel')
    hold on
