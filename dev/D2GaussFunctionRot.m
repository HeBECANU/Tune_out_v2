...
%from https://au.mathworks.com/matlabcentral/fileexchange/37087-fit-2d-gaussian-function-to-data
%% x = [Amp, x0, wx, y0, wy, rot,offset]
%[X,Y] = meshgrid(x,y) 
%  xdata(:,:,1) = X
%  xdata(:,:,2) = Y           
...
F = x(7)+ x(1)*exp(   -((xdatarot(:,:,1)-x0rot).^2/(0.5*x(3)^2) + (xdatarot(:,:,2)-y0rot).^2/(0.5*x(5)^2) )    );
