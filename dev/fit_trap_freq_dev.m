function osc_fit=fit_trap_freq_dev(anal_opts_osc_fit,data)
...
osc_fit.model_coefs=nan(iimax,10,2);
...
%         y_tmp=y_tmp-ac_tmp*0;%3e-4;
        ...
        % optimizer
        modelfun = @(b,x) exp(-x(:,1).*max(0,b(7))).*b(1).*...
        ...
        gf_opt.start=[fit_amp, fit_freq, fit_phase, fit_offset,0,0,2,0,0,0];
        gf_opt.rmse_thresh=0;%2e-6;
        gf_opt.plot=false;
...
%       
        ...
        cof_names={'amp','freq','phase','offset','xcpl','zcpl','damp','grad','chirp','chirp2'};
        ...
        if anal_opts_osc_fit.plot_fits
            ...
            abs_fft_resid=abs(fft_resid(2,:));
            fprintf('max residual fft peak amp %e mm/s \n',max(abs_fft_resid)*1e3)
            plot(fft_resid(1,:),abs_fft_resid,'k')
            hold on
            ...
            subplot(4,1,4)
            fft_x=fft_tx(t_tmp,x_tmp,'padding',10);
            plot(fft_x(1,:),abs(fft_x(2,:)),'k')
            hold on
            fft_y=fft_tx(t_tmp,y_tmp,'padding',10);
            plot(fft_y(1,:),abs(fft_y(2,:)),'r')
            fft_z=fft_tx(t_tmp,z_tmp,'padding',10);
            plot(fft_z(1,:),abs(fft_z(2,:)),'b')
            hold off
