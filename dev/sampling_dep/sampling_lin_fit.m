...
beta0=[grad_truth,yintercept_truth];
...
for idxa=1:iimax
    ...
    for idxb=1:repeat_fit
        ydat=linmodel(beta0,xval);
        ydat=ydat+normrnd(0,noise_amp,size(ydat,1),1);
        opts = statset('nlinfit'); %,'Options',opts
        fit_mdl = fitnlm(xval,ydat,linmodel,beta0);
        xint_samp=-fit_mdl.Coefficients.Estimate(2)/fit_mdl.Coefficients.Estimate(1);
        est_fit_param(idxb,:)=[fit_mdl.Coefficients.Estimate(1),xint_samp,fit_mdl.Coefficients.Estimate(2)];
        if plot_fits
            sfigure(1);
            set(gcf,'color','w')
            subplot(2,1,1)
