...
%%input masking
is_cal=data.mcp_tdc.probe.calibration'; %because its used a lot make a temp var for calibration logic vector
is_cal(isnan(is_cal))=1;    
probe_dat_mask=data.osc_fit.ok.all & ~is_cal &  ~isnan(data.wm_log.proc.probe.freq.act.mean)'...
...
%% output the data
data_signal=[];
data_signal.square_probe_freq.val=nan*is_cal;
data_signal.square_probe_freq.val(probe_dat_mask)=square_trap_freq;
data_signal.square_probe_freq.unc(probe_dat_mask)=square_trap_freq_unc;
