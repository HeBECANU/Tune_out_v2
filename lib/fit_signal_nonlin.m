function to_res=fit_signal_nonlin(anal_opts_fit_to,data)
...
[to_freq,mdl_all]=fit_poly_data(xydat);
...
[to_freq,mdl_culled]=fit_poly_data(culed_xydat);
...
to_res.fit_trimmed.to_unc_fit_no_cov=abs((mdl_coef.Estimate(1)/mdl_coef.Estimate(2)))*...
    sqrt(...
...
boot=bootstrap_se(@fit_poly_data,culed_xydat,...
    'plots',true,...
    'replace',true,...
...
%% See how the intercept error changes with scan size
scan_radius=min([abs(min(xdat_culled)-to_freq),max(xdat_culled)-to_freq]);
ii = 1;
delta_c=zeros(1,9);
for scan_p = 0.2:0.1:1.0
    x_trunc=linspace(to_freq-scan_p*scan_radius,to_freq+scan_p*scan_radius,60);
    y_trunc=polyval(mdl_coef,x_trunc);
    temp_mdl=fitlm(x_trunc,y_trunc,'linear');
    temp_to_freq=-temp_mdl.Coefficients.Estimate(1)/temp_mdl.Coefficients.Estimate(2);
    delta_c(ii)=to_freq-temp_to_freq;
    ii = ii +1;
end
...
function [to_freq,fit_mdl]=fit_poly_data(in)
inmat=cell2mat(in);
xdat=inmat(1,:);
ydat=inmat(2,:);
coeff_check=true;
...
n=2;
while coeff_check
    mdl_n = strcat('poly',num2str(n));
    fit_mdl = fitlm(xdat,ydat,mdl_n);
    if fit_mdl.Coefficients{n-2:n,4}>p_tol
        coeff_check=false;
    end
    n = n+1;
end
mdl_zeros = roots(mdl.Coefficients{:,4});
[c, indx] = min(abs(mdl_zeros-422));
