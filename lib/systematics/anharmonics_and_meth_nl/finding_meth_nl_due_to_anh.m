...
power=50e-3;
detuning_hz=-12000e9;
probe_trap_freq_sq_grad=( 1/(4*(pi^2)) )*(1/(2*const.epsilon0*const.c*const.mhe))...
                    *dpolz_domega...
                    *( (2*power)/(pi*(opts_template.gauss_radius^2)) ) ...
                    *( (4)/(opts_template.gauss_radius^2) ) ;
abs(sqrt(probe_trap_freq_sq_grad*detuning_hz))
abs(sqrt(probe_trap_freq_sq_grad*detuning_hz))/opts_template.trap_freq
