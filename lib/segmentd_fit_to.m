...
function to_res_seg=segmentd_fit_to(anal_opts_fit_to,data)
...
to_res_seg.num_shots=sum(probe_dat_mask);
to_res_seg.fit_mask=probe_dat_mask;
...
shot_time=data.mcp_tdc.time_create_write(probe_dat_mask,2);
...
for ii=1:iimax
    ...
    seg_mask=seg_time_start<shot_time & shot_time<seg_time_start+segment_width;
    ...
    to_res_seg.fit_all.to_freq(ii)=to_freq/anal_opts_fit_to.scale_x+freq_offset;
    to_res_seg.fit_all.model{ii}=mdl_all;
    to_res_seg.freq_offset(ii)=freq_offset;
    ...
    to_res_seg.fit_trimmed.to_freq(ii)=to_freq/anal_opts_fit_to.scale_x+freq_offset;
    to_res_seg.fit_trimmed.model{ii}=mdl_culled;
    mdl_coef=mdl_culled.Coefficients;
    to_res_seg.fit_trimmed.to_unc_fit(ii)=abs((mdl_coef.Estimate(1)/mdl_coef.Estimate(2)))*...
    ...
    to_res_seg.fit_trimmed.boot{ii}=boot;
    to_res_seg.fit_trimmed.to_unc_boot(ii)=boot.se_opp/anal_opts_fit_to.scale_x;
    to_res_seg.fit_trimmed.single_shot_uncert_boot(ii)=to_res_seg.fit_trimmed.to_unc_boot(ii)*sqrt(numel(xdat_culled));
...
plot(to_res_seg.fit_all.to_freq)
end
