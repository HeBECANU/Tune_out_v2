...
loop_config.dir = {
    ...
    'Z:\EXPERIMENT-DATA\2018_Tune_Out_V2\unsorted\filt_dep\20190218_filt_dep_3\',
...
%%
sfigure(3000);
plot_offset = 725736106677507;%sum(data.drift.to_val{1}.*data.drift.to_val{2})/sum(data.drift.to_val{2});
filt_num = [0,1,1,2,3,2,3,1];
errorbar(filt_num,(data.main.quad.to.val-plot_offset).*1e-6,data.main.quad.to.unc.*1e-6,'kx')
set(gcf,'color','w')
...
to_freqs_val = data.main.lin.to.val./1e6; %convert to blue
to_freqs_err = data.main.lin.to.unc./1e6; %convert to blue
...
sfigure(8320);
disp_config.colors_main = [[233,87,0];[33,188,44];[0,165,166]];%[[75,151,201];[193,114,66];[87,157,95]];
...
plot_binned_nice(disp_config,filt_num,to_freqs_val,wlin,fit_mdl_lin)
 ylim([-90 90])
