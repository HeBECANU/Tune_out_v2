...
% subdirs = {'20190211T1737_to_hwp_333.5_polmin_168.5_nuller_reconfig_pdset_0.2v';
...
%     '20190214T1220_to_hwp_333.5_polmin_168.5_nuller_reconfig_pdset_0.6v';
...
loop_config.dir = cellfun(@(x) fullfile(data_root,x),subdirs,'UniformOutput',0);
...
grads_val = data.drift.grad.val % ./2; % bmh: why is this divided by two ?
grads_unc = data.drift.grad.unc % ./2;
% grads_val = 1e9*data.drift.grad.val;
% grads_unc = 1e9*data.drift.grad.unc;

to_freq = data.drift.to.val .* 1e-6;
to_freq_unc = data.drift.to.unc .* 1e-6;

% grads_avg = data.main.grad %./2; % bmh: why is this divided by two ?
to_vals_run = data.main.lin.grad.val;
to_vals_run_unc =  data.main.lin.grad.unc;

%%


grad_cut = ~(grads_val<=0);

grads_val = grads_val(grad_cut);
grads_unc = grads_unc(grad_cut);
to_freq = to_freq(grad_cut);
to_freq_unc = to_freq_unc(grad_cut);
...
for bidx = 1:num_bins
    mask = grads_val > grad_edges(bidx) & grads_val < grad_edges(bidx+1);
    if sum(mask)>0
        to_mean(bidx) = nanmean(to_freq(mask));
    %     to_se(bidx) = mean(to_vals_unc(mask));
        to_se(bidx) = std(to_freq(mask));
    end
end
nmask = ~isnan(to_mean);
...
% grad_pow = 2;
weights = 1./to_freq_unc.^2;
weights = weights/sum(weights);
% weights = grads.;
mdl_fun = @(b,x) b(1)+b(2).*x(:,1);
beta0 = [nanmean(to_freq),1e5];
opts = statset('MaxIter',1e4);
% full fit
cli_header('UNWT FIT ALL DATA');
[full_mdl,gof,fit_out] = fit(grads_val,to_freq,'poly1')
full_ci=confint(full_mdl,1-erf(1/sqrt(2)))-[full_mdl.p1,full_mdl.p2];
data_pi = predint(full_mdl,grads_val,1-alpha_cull,'observation','off');
...
% full weighted fit
[wt_mdl,wt_gof,wt_out] = fit(grads_val,to_freq,'poly1','Weights',weights)%'ErrorModel','combined'
...
[mask_mdl,mask_gof,mask_out] = fit(grads_val(outlier_mask),to_freq(outlier_mask),'poly1','Weights',mwts)
mask_mdl_full= fitlm(grads_val(outlier_mask),to_freq(outlier_mask),'Weights',mwts)
...
% weighted culled fit
[uwt_mask_mdl,uwt_mask_gof,uwt_mask_out] = fit(grads_val(outlier_mask),to_freq(outlier_mask),'poly1')
uwt_mask_mdl_full = fitlm(grads_val(outlier_mask),to_freq(outlier_mask))
uwt_mask_ci=confint(uwt_mask_mdl,0.95)-[uwt_mask_mdl.p1,uwt_mask_mdl.p2];
uwt_mask_mdl_full.ModelCriterion

sum((mask_mdl(grads_val(outlier_mask))-to_freq(outlier_mask)).^2)
sum((uwt_mask_mdl(grads_val(outlier_mask))-to_freq(outlier_mask)).^2)
...
grads_cull = grads_val(outlier_mask);
to_vals_cull = to_freq(outlier_mask);
to_vals_unc_cull = to_freq_unc(outlier_mask);
for bidx = 1:num_bins
...
pred_vals=full_mdl(grads_val);
pred_PI = predint(full_mdl,grads_val,erf(1/sqrt(2)),'observation','off')-pred_vals;
...
pred_vals=wt_mdl(grads_val);
pred_PI = predint(wt_mdl,grads_val,alpha,'observation','off')-pred_vals;
pred_std = mean(abs(pred_PI),2);
...
pred_PI = predint(uwt_mask_mdl,grads_val(outlier_mask),erf(1/sqrt(2)),'observation','off')-uwt_mask_mdl(grads_val(outlier_mask));
pred_std = mean(abs(pred_PI),2);
...
pred_PI = predint(mask_mdl,grads_val(outlier_mask),erf(1/sqrt(2)),'observation','off')-mask_mdl(grads_val(outlier_mask));
pred_std = mean(abs(pred_PI),2);
% chi_square = sum(mwts.*mask_out.residuals.^2./pred_std.^2);
% chi_square_per_dof_mask_wt= chi_square/mask_gof.dfe
 
check_mask = grads_val > 25 & grads_val < 35;
check_grad = grads_val(check_mask);
to_check = to_freq(check_mask);
...
all_pts=errorbar(grads_val(outlier_mask),to_freq(outlier_mask)-to_offset,to_freq_unc(outlier_mask),'o','MarkerSize',2,'Color',colors_main(3,:),...
...
all_pts=errorbar(grads_val(outlier_mask),to_freq(outlier_mask)-to_offset,to_freq_unc(outlier_mask),'o','MarkerSize',6,'Color',colors_main(col_use,:),...
