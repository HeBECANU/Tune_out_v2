...
power_setpt_list = linspace(2.6,0,50);
...
if mod(i,calibrate_interval)==0
    ...
    new_path='c:\remote\settings201917Jan112143.xml'; %Lieutenant Fangtooth
    %write log entry
    ...
    new_path='c:\remote\settings201914Feb193925.xml'; %Lieutenant Fangtooth
    ...
    % each line will be a new cell in set_file_lines
    ...
    set_file_lines={};
    line_read='';
    while ischar(line_read)
        j = j+1;
        line_read = fgetl(file_id);
        set_file_lines{j} = line_read;
    end
    fclose(file_id);
    % make adjustments
