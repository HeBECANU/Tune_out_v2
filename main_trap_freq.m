...
% - Match up the Labview data with the tdc data
...
%the data structure
%   first level is instrument or analysis method
%   try and pass things between the modules of code only using the 'data' struct
%   keep everything referenced to data.mcp_tdc eg. the probe beam power ok should corespond to 

% TIMING 
% because there are so many moving pieces a lot of the script requires
% matching up the times of the various inputs:
...
root_data_dir='E:\Data\Tuneout\tune_out_example_data';
...
% % Changes for JR WFH ---------------------------------------------------
% Will save cache on local SSD instead of external HDD for speed of
% retrieval
anal_opts.tdc_import.save_cache_in_data_dir=false;
anal_opts.tdc_import.force_load_save=false;   %Do I want this true?
% What about that reprocess_folder option?
...
for dir_idx = selected_dirs
    ...
    % JR: May want to change first argument to custom cache dir. 
    if should_process_folder(anal_opts.tdc_import.dir,reprocess_folder_if_older_than,active_process_mod_time)  
        try
            ...
            beam_power.val = 150e-3;
            beam_power.unc = beam_power.val*mean(PD_rel_unc); %for now
            ...
            % and thus we have the gradient with respect to f (in Hz?)
            data.to_fit_seg.fit_trimmed.d_alpha_d_f.val = (beam_waist.val.^4)/(4*A*beam_power.val).*data.to_fit_seg.fit_trimmed.slope.val;
            data.to_fit_seg.fit_trimmed.d_alpha_d_f.unc = sqrt( (beam_power.unc/beam_waist.val^4)^2 + ...
                                           (4*beam_power.val*beam_waist.unc*beam_waist.val.^-5).^2)/(4*A);
                                       %this ca
            %using the figure as a guide we have roughly 2e-41 (C m^2/V)
            % per THz. So from slopes we get
%             data.to_fit_seg.fit_trimmed.d_alpha_d_f.val*1e12 
            % two orders of mag off the paper figure - maybe a polz
            % dependent thing?
